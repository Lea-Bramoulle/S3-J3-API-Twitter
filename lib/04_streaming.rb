...
def test_tweet (client)
	...
	client.update('Mon premier tweet en Ruby (french) !!!!')
end

def hello_journalists(client)
	5.times do
		random = rand(LIST_JOURNALISTS.size)
		client.update("#{LIST_JOURNALISTS[random]} Thanks for your work ! #bonjour_monde ")
		puts "the bot sent an tweet to #{LIST_JOURNALISTS[random]} #bonjour_monde"
	end
end
...
def stream (clientStream,client)
	clientStream.filter(track: "#Blizzardboycott") do |object|
